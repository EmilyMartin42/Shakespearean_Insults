...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="xsl:initial-template">
        <xsl:result-document href="../../webContent/images/maximXilloc.svg">
            <svg viewBox="-75 100 1000 720" width="100%">
                <g transform="translate(100, {$max-wide + 200})">
                    ...
                    <xsl:for-each-group select="$all_plays//insult" group-by="insultStart/@maxim">
                        <xsl:variable name="y-pos" as="xs:double" select="($spacing * 2) + (position() - 1) * ($spacing * 2)"/>
                        <xsl:for-each select=".">
                    ...
                    <xsl:for-each-group select="$all_plays//insult" group-by="insultStart/@illoc-type">
                        <xsl:variable name="x-pos" as="xs:double" select="($spacing * 2 + 50) + (position() - 1) * ($spacing * 2)"/>
                        <xsl:for-each select=".">
                            <line x1="{$x-pos}" x2="{$x-pos}" y1="0" y2="-{$max-high + 50}" stroke="#E0E0E0" stroke-dasharray="4"/>
                            <text x="{$x-pos}" y="20" stroke="#E0E0E0" fill="#E0E0E0" text-anchor="middle">
                        ...
                        <xsl:for-each-group select="current-group()" group-by="insultStart/@maxim">
                            ...
                            <xsl:variable name="y-pos" as="xs:double" select="($spacing * 2) + (position() - 1) * ($spacing * 2)"/>
                            <xsl:variable name="r" as="xs:double" select="10 * math:sqrt($area div math:pi())"/>
                            <xsl:for-each select=".">
                                <circle cx="{$x-pos}" cy="-{$y-pos}" r="{15 * math:sqrt($area div math:pi())}" fill="#5D81B9" opacity=".75"/>
