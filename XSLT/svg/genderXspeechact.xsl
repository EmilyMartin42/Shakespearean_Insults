...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template  name="xsl:initial-template">
        <svg height="1000" width="1000">
            <g transform="translate(50, {$max-high + 200})">
                <!-- axis -->
                ...
                <xsl:for-each select="(1 to ($max-high div 40))">
                    <line x1="50" y1="-{. * $y-scale * 10}" x2="{$max-wide}" y2="-{. * $y-scale * 10}"
                        stroke="black" stroke-dasharray="3"/>
                    <text x="25" y="-{. * ($y-scale * 10) - 5}"><xsl:value-of select=". * 10"/></text>
                </xsl:for-each>               
                <!-- labels -->
                <text x="75" y="-{$max-high + 130}" font-size="18">Direct vs. Indirect insult counts between male speakers</text>                
                ...
                <xsl:for-each-group select="$all_plays//insult" group-by="insultStart/@gender">
                    ...
                    <xsl:for-each-group select="current-group()"
                        ...
                        <g>
                            ...
                            <xsl:for-each select=".">
                                <rect x="100" y="-{$sA-count * $y-scale}" width="{$bar-wide}" height="{$sA-count * $y-scale}" fill="green"/> 
