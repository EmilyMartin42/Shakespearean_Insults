...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="insultStart" mode="insult">
        ...
        <!-- Insult is start, words and punctuation before end, end       -->
        <!-- ============================================================ -->
        <xsl:variable name="myEnd" as="element(insultEnd)" select="following::insultEnd[1]"/>
        <xsl:variable name="myInsult"
            select="., (following::w | following::punc)[. &lt;&lt; $myEnd], $myEnd"/>
        <insult>
